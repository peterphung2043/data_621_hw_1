...
```{r }
set.seed(123)
original_split <- sample.split(original_train_data$TARGET_FLAG, SplitRatio = 0.8)
original_train <-  subset(original_train_data, original_split == TRUE)
original_test <- subset(original_train_data, original_split == FALSE)

table(original_test$TARGET_FLAG)
table(original_train$TARGET_FLAG)

modified_split <- sample.split(insurance_train_data$TARGET_FLAG, SplitRatio = 0.8)
modified_train <-  subset(insurance_train_data, modified_split == TRUE)
modified_test <- subset(insurance_train_data, modified_split == FALSE)
...
```{r}
logit_original_vars <- glm(formula = TARGET_FLAG ~ . - TARGET_AMT, data = original_train, family = "binomial" (link="logit"))
