...
```{r}
#required libraries
packages <- c("RCurl", "tidyr", "dplyr", "reshape2", "corrplot", "mice", "car", "reshape", "mixtools", "tidyverse", "GGally", "MASS", "regclass", "knitr", "kableExtra")
```
...
```{r}
#view training data set first 6 rows
kable(head(crime_training, 6)) %>%
...
**showing a bionomial distribution**
...
crime_training %>% 
  ...
  labs(title = "Neighborhood Crime Target Data",
       ...
       caption = "DATA 621",
       tag = "Binomial Distribution Plot",
       y = "factors",
       x = "Target") +
  theme_bw() 
```
...
**Frequency distribution of the possible number of successful outcome in a given number of trials in each of which there is the same probability of success**
```{r}
train_count <- crime_training %>%
  count(target) %>%
...
**converting the variables class can be performed later in the analysis**
```{r, comment = NA}
#view data set variables summary statistics
