...
```{r}
log_reg <- glm(target ~ ., data = crime_training, family = binomial(link = "logit"))
summary(log_reg)
```
...
```{r}
logodds <- log_reg$linear.predictors
boxTidwell(logodds ~ crime_training$nox)

plot(logodds ~ crime_training$nox)
...
crime_training <- append_bimodal_dummy_var(550, "tax", crime_training)
```
